...
Usage:
    python scripts/preprocess_da3_chunked.py \\
...
def main():
    ...
    p.add_argument("--chunk_size", type=int, default=0,
                   help="Optional number of frames to process per DA3 forward pass. "
                        "0 means all frames at once.")
    ...
    chunk_size = args.chunk_size if args.chunk_size and args.chunk_size > 0 else len(pil_images)
    depth_chunks = []
    extr_chunks = []
    intr_chunks = []
    with torch.no_grad():
        for start in range(0, len(pil_images), chunk_size):
            end = min(len(pil_images), start + chunk_size)
    ...
    intr_proc = np.concatenate(intr_chunks, axis=0)     # (T, 3, 3) at H_proc x W_proc
